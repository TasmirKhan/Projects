...
const s=useAgentStore(); const [live,setLive]=useState<string[]>([]);
useEffect(()=>{if(!s.stages.length) s.setStages(getInitialStages());},[s]);
const run = async()=>{if(!s.task.trim()) return; s.setRunning(true); setLive([]); let local=getInitialStages(); s.setStages(local);
...
{s.isRunning && <div className='text-xs text-zinc-400 animate-pulse'>Autonomous execution in progress...</div>}
...
<div className='card'><h3>Streaming Execution Feed</h3><ul className='mt-2 text-sm space-y-1'>{live.map((l,i)=><li key={i}>• {l}</li>)}</ul></div>
